...
class FindProvider
{
    ...
    public function calcDebosit($id, $date){
        ...
        $q = \DB::connection('mysql1')->select("with cte as (select IFNULL(SUM(amount),0) as prix, 0 as ras from tb_gas_debit where id_abonent = $id and dt_pay <= '$date' union all select 0, IFNULL(SUM(price),0) from tb_requests_ballons where id_abonent = $id and dt <= '$date') select SUM(prix) - SUM(ras) as deposit from cte");
